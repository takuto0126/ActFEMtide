...
boundary top       !$bt
##--- Details for the second slice -----
## second slice file will be ${ID}_03.dat
## slice x = 0 (SN vertical slice)
A                  !0.0
B                  !0.0
D                  !20.0
boundary left      !-100.0
boundary right     !100.0
boundary bottom    !-100.0
boundary top       !100.0
...
gmt makecpt -Crainbow -T1.0/3.5/0.1 -I
#gmt makecpt -Crainbow -T0/3.0/0.01 -I

# WE slice
gmt basemap -JX$scl -R$range -Bxa50We+l"Easting [km]" -Bya10Sn+l"Elevation [km]" -Y8
...
# SN slice
gmt basemap -JX$scl -R$range -Bxa50We+l"Northing [km]" -Bya20Sn+l"Elevation[ km]" -Y-6.5
...
# Horizontal slice
gmt basemap -JX8/8 -R-100/100/-100/100 -Bxa50We+l"Easting [km]" -Bya50Sn+l"Northing [km]" -X9.5
...
-0.15 0.50
EOF
echo "0.5  7.5 $ID z=-20km" | gmt text -JX8/8 -R0/8/0/8  -Gwhite -F+f10p,Helvetica,black+jLM

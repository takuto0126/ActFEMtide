...
type(model),          intent(inout) :: g_model
  ...
  CALL SHAREMODEL(g_model,ip)                 ! see m_shareformpi.f90  2018.03.19 for iactive
...
integer(4)                :: nmodelactive   ! 2018.03.19

 call MPI_BCAST(h_model%nmodel,      1, MPI_INTEGER4,0,MPI_COMM_WORLD,errno)
 call MPI_BCAST(h_model%nmodelactive,1, MPI_INTEGER4,0,MPI_COMM_WORLD,errno) ! 2018.03.19
 ...
 nmodelactive = h_model%nmodelactive ! 2018.03.19
 ...
 if ( ip .ne. 0) then
  ...
  if ( .not. allocated(h_model%iactive)     ) allocate( h_model%iactive(     nmodel)) ! 2018.03.19
 end if
 ...
 call MPI_BCAST(h_model%iactive     , nmodel, MPI_INTEGER4,0,MPI_COMM_WORLD,errno) ! 2018.03.19
...
integer(4)                          :: nsr_inv, nobs_s ! 2017.07.13
 ...
 do j=1,nsr_inv       ! 2017.07.13
  call MPI_BCAST(g_param_inv%obsinfo(j)%nobs_s,1,MPI_INTEGER4, 0,MPI_COMM_WORLD,errno) !2017.07.13
  nobs_s = g_param_inv%obsinfo(j)%nobs_s ! 2017.07.13
  if ( ip .ne. 0) then ! 2017.07.13
   allocate(g_param_inv%obsinfo(j)%obsfile(nobs_s) ) ! 2017.07.13
   allocate(g_param_inv%obsinfo(j)%obsindex(nobs_s)) ! 2017.07.13
  end if               ! 2017.07.13
  do i=1,nobs_s        ! 2017.07.13
  call MPI_BCAST(g_param_inv%obsinfo(j)%obsfile(i),50,MPI_CHAR, 0,MPI_COMM_WORLD,errno)!2017.07.13
  call MPI_BCAST(g_param_inv%obsinfo(j)%obsindex(i),1,MPI_INTEGER4,0,MPI_COMM_WORLD,errno)!2017.07.13
  end do         ! 2017.07.13
 end do          ! 2017.07.13
...
if (ip .eq. 0 ) write(*,'(a)') " ### SHAREMESHLINE END!! ###"!2020.09.18
...
if ( ip .eq. 0) write(*,'(a)') " ### SHAREMESH END!! ###" !2020.09.18
...
!#[3]## broadcast
 ...
 !# for ocean mesh node 2024.03.16
 call MPI_BCAST(g_param%nodes,    1, MPI_INTEGER4,0,MPI_COMM_WORLD,errno)

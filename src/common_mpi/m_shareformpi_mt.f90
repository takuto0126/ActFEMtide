!# coded on 2025.09.16
module shareformpi_mt
use param_mt
use surface_type
use outresp     ! 2025.09.17
use mesh_type   ! 2025.09.17
use line_type   ! 2025.09.17
use mpi         ! 2025.09.16
implicit none
...
!#[3]## set output
  g_freq_mt%nfreq    = nfreq
  g_freq_mt%nfreq_ip = nfreq_ip
  allocate( g_freq_mt%freq_ip(nfreq_ip) )
  allocate( g_freq_mt%ifreq_ip(nfreq_ip) )
  allocate( g_freq_mt%freq   (nfreq)    )
  g_freq_mt%freq     = freq
  g_freq_mt%freq_ip  = freq_ip
  g_freq_mt%ifreq_ip = ifreq_ip
...
subroutine sharemt(g_param_mt,g_surface,ip)
 ! coded on 2021.12.30
  use surface_type ! ../src_3DMT/m_surface_type.f90
...
subroutine sharemesh(g_mesh,ip)
 ! moved from m_shareformpi_joint.f90 on 2025.09.17
 implicit none
 integer(4),intent(in)    :: ip
...
subroutine sharerespdata(resp,ip_from)
 ! moved from src_inv_joint/m_shareformpi_joint.f90 on 2025.09.17
 implicit none
...
!############################################################
! moved from ../src_inv_joint/m_shreformpi_joint.f90 on 2025.09.16

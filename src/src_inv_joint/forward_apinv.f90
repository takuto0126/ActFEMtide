sources!# modified on 2017.08.31 for multiple sources amp phase inversion
!# Copied from ../src_inv/forward_inv.f90
!# Coded on March 4, 2016
!# copied originally from ../solver/forward_bxyz.f90
subroutine forward_apinv(A,h_mesh,l_line,doftot,nsr,bs,omega,sparam,g_param,&
                   &     g_param_apinv,g_cond,PT,ut,ip,np) ! 2017.09.03
use mesh_type
use iccg_var_takuto
use line_type
use param
...
type(param_apinversion), intent(in)     :: g_param_apinv ! m_param_apinv 2017.08.31
type(param_cond)  ,      intent(in)     :: g_cond
type(real_crs_matrix),   intent(in)     :: PT(5)         ! 2018.10.05 Bx,By,Bz,Ex,Ey
complex(8),              intent(out)    :: bs(doftot,nsr)! 2017.07.13
type(global_matrix),     intent(inout)  :: A             ! see m_iccg_var_takuto.f90
type(complex_crs_matrix),intent(out)    :: ut(5)         ! 2018.10.05
...
complex(8), allocatable,dimension(:,:)  :: b_vec   ! 2017.07.13
integer(4)                              :: i,j,k,IB
...
allocate(b_vec(doftot,nsr))                  ! 2017.07.13
...
CALL INITIALIZE(A,b_vec,bs,doftot,nsr)       ! nsr is added 2017.07.13

!#[4-2]## Generate Matrix for CRS format
CALL GENMAT(h_mesh,l_line,A,b_vec,omega,sparam,g_param,g_cond,g_param_apinv) ! 2017.08.31
...
!#[6-1]## for CG
CALL SET_BC_ICCG(A, doftot, nsr, b_vec, Avalue_bc, line_bc) ! 2017.07.13

!#[7]## Solve
call solvePARDISOinv(doftot,nsr, A,b_vec,bs,PT,ut,iflag_comp,ip,np) ! 2020.09.18

call watchstop(t_watch) ! 2017.12.22
write(*,'(a,i2,a,i2,a,f9.4,a)') " ### forward_apinv  END !! ###  ip =",ip," /",np," Time =",t_watch%time," [min]" ! 2020.09.18

return
end subroutine forward_apinv              ! 2017.08.31
!########################################
! modified on 2017.07.13 for multiple sources
subroutine initialize(A,b_vec,bs,doftot,nsr)
use iccg_var_takuto ! global_matrix
implicit none
type(global_matrix)    :: A
integer(4),intent(in)  :: doftot
integer(4),intent(in)  :: nsr    ! 2017.07.13
complex(8),intent(out) :: b_vec(doftot,nsr),bs(doftot,nsr) ! 2017.07.13

A%D(:)    =(0.d0,0.d0)
A%AU(:)   =(0.d0,0.d0)
A%AL(:)   =(0.d0,0.d0)
bs(:,:)   =(0.d0,0.d0) ! 2017.07.13
b_vec(:,:)=(0.d0,0.d0) ! 2017.07.13
...
subroutine GENMAT(h_mesh,l_line,A,b_vec,omega,sparam,g_param,g_cond,g_param_apinv)
...
real(8),                intent(in)    :: omega
complex(8),             intent(out)   :: b_vec(l_line%nline,g_param_apinv%nsr_inv)!20170713
...
!#[0]## set
  ...
  srcindex = g_param_apinv%srcindex  ! 2017.08.31
  ...
  do j=1,nsr_inv  ! 2017.07.13
   ...
   if (itrue) then ! when the cell includes the wire current
    ...
    do i=1,6
     ...
     b_vec(ii,j)=b_vec(ii,j) + rhs1  ! 2017.07.13
    end do
...
SUBROUTINE SET_BC_ICCG(A,doftot,nsr,rf,dirichlet_value,dirichlet) !2017.07.13
use iccg_var_takuto
implicit none
type(global_matrix),          intent(inout) :: A
integer(4),                   intent(in)    :: doftot
...
!#[3]## modify stiffness matrix
      ...
      enddo
!write(*,*) "### SET_BC_ICCG END!! ###" 2017.12.22
RETURN
END
!
!

...
!##[mt_imp_ap_flag]
 ...
 !#[4]## read mt data
   call readdata_mt(g_param_joint,g_data_mt)! see below
...
!#################################################### readdata_mt
subroutine readdata_mt(g_param_joint,g_data_mt) ! 2022.01.02
implicit none
  ...
  do k=1,4 ! zxx -> zyy
   do l=1,2 ! real, imag
    if( data_avail(l,k,j,i) ) then
     ii=ii+1
     dvec_mt(ii) = data(l,k,j,i)
      err_mt(ii) =  err(l,k,j,i)

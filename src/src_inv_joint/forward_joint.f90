!# modified on 2017.08.31 for multiple sources amp phase inversion
...
type(param_joint),       intent(in)     :: g_param_joint ! 2021.12.25
...
complex(8), allocatable,dimension(:,:)  :: Avalue_bc_mt   ! 2021.12.29
logical,    allocatable,dimension(:,:)  :: line_bc_mt     ! 2021.12.29
complex(8), allocatable,dimension(:,:)  :: rf,rf_mt       ! 2021.12.29 rhs vectors
integer(4)                              :: i,j,k,IB
real(8)                                 :: zmin,zmax,xout,yout
type(watch)                             :: t_watch ! 2017.12.22
call watchstart(t_watch) !2017.12.22

!#[1]## set
!ijoint = g_param_joint%ijoint  ! 2022.10.14
!call setnec(ijoint,ACT,MT)        ! 2022.10.14 see n_inv_joint.f90
allocate(Avalue_bc(   nline  ),line_bc(   nline  ) )
allocate(Avalue_bc_mt(nline,2),line_bc_mt(nline,2) ) ! 2021.12.29
allocate(rf(    nline,nsr) )
allocate(rf_mt( nline,2  ) )  ! 2021.12.29
ut = null ! initialize (2017.05.16)
...
!#[6-1]## for CG
if (MT ) CALL SET_BC_3DMT(A, nline,2, rf_mt, Avalue_bc_mt, line_bc_mt(:,1), ip) ! 2022.10.14
if (ACT) CALL SET_BC_ACTIVE(A, nline, nsr, rf, Avalue_bc, line_bc)              ! 2022.10.14
...
subroutine initialize_joint(ACT,MT,A,rf,rf_mt,bs,bs_mt,nline,nsr)
...
integer(4),         intent(in)    :: nline
integer(4),         intent(in)    :: nsr    ! 2017.07.13
complex(8),         intent(inout) :: rf(nline,nsr),bs(nline,nsr) ! 2017.07.13
complex(8),         intent(inout) :: rf_mt(nline,2),bs_mt(nline,2) ! 2021.12.29

A%D(:)    =(0.d0,0.d0)
A%AU(:)   =(0.d0,0.d0)
A%AL(:)   =(0.d0,0.d0)
if (ACT) bs(:,:)   =(0.d0,0.d0) ! 2017.07.13
if (ACT) rf(:,:)   =(0.d0,0.d0) ! 2017.07.13
if (MT)  bs_mt(:,:)=(0.d0,0.d0) ! 2021.12.29
if (MT)  rf_mt(:,:)=(0.d0,0.d0) ! 2021.12.29
...
SUBROUTINE SET_BC_3DMT(A,nline,nsr,rf,dirichlet_value,dirichlet,ip)
  use iccg_var_takuto
  implicit none
  type(global_matrix), intent(inout) :: A
  ...
  END
  !###############################################  SET_BC_ACTIVE
! nsr for multiple sources are added on 2017.07.13
! ip is deleted on 2017.05.15
! This program is based on set_bc_iccg.f90 in GeoFEM
! Coded on Aug. 21, 2015
SUBROUTINE SET_BC_ACTIVE(A,doftot,nsr,rf,dirichlet_value,dirichlet) !2017.07.13
  use iccg_var_takuto
  implicit none
  type(global_matrix),          intent(inout) :: A
  ...
  complex(8),                   intent(out)   :: rf(doftot,nsr)   ! 2017.07.13
  integer(4)      :: i, j, ipos
  complex(8)      ::  temp
  !#[1]## Initialize
  !rf(1:doftot)=(0.d0, 0.d0)
  ...
  !         direchlet : {rf} =            {dirichlet_value}
        do i=1,doftot
          if (dirichlet(i)) then
       do j=1,nsr ! 2017.07.13
            rf(i,j) = dirichlet_value(i) ! 2017.07.13
       end do     ! 2017.07.13
          else
            ...
            enddo
         do j=1, nsr ! 2017.07.13
              rf(i,j) = rf(i,j) - temp ! 2017.07.13
         end do      ! 2017.07.13
          endif
        enddo
  
  !#[3]## modify stiffness matrix
        if ( .false.) then ! 2021.12.31 because to keep A for ACTIVE BC
        do i=1,doftot
          if (dirichlet(i)) then
                A%D   (i   ) = 1.d0
            do ipos=A%INL(i-1)+1,A%INL(i)
  ...
  !write(*,*) "### SET_BC_ACTIVE END!! ###" 2017.12.22
  RETURN
  END
!###############################################  SET_BC_3DJoint
! modified for 3DMT on 2021.09.14
! assumes the lines for dirichlet boundary are common for all the MT and CSEM problems
! Coded on Aug. 21, 2015
SUBROUTINE SET_BC_3DJoint(A,nline,nsr,rf,rf_mt,dvalue,dvalue_mt,dirichlet,ip)
  use iccg_var_takuto
  implicit none
  type(global_matrix),intent(inout) :: A
  integer(4),         intent(in)    :: nline,ip,nsr      ! nsr : ACTIVE sources
  complex(8),         intent(in)    :: dvalue(nline)  ! 2021.012.31
  complex(8),         intent(in)    :: dvalue_mt(nline,2)! 2021.12.29
  logical,            intent(in)    :: dirichlet(nline)  !whethe dirichlet boundary
  complex(8),         intent(inout) :: rf(nline,nsr)   ! 2021.09.16
  complex(8),         intent(inout) :: rf_mt(nline,2)  ! 2021.09.29
  !---------------  internal variables -------------------
  integer(4)  :: i, j, ipos,nsr_mt=2 ! 2021.12.29
  complex(8)  :: rf_tmp(nline,nsr+2),dvalue_tmp(nline,nsr+2)
  complex(8)  ::  temp
  !#[1]## Initialize
  !rf(1:nline)=(0.d0, 0.d0)
  !nline=nodtot
  !dirichlet(:)=.false.
  !dvalue(:)=(0.d0,0.d0)
  !do i=1,nline_bc
  !  dirichlet(line_bc(i))=.true.
  !  dvalue(line_bc(i))=Avalue_bc(i)
  !end do
  !do i=1,nline
  !write(*,*) "dvalue(i)=",dvalue(i),"i=",i
  !end do
  !#[2]##  modify right-hand side vector
  !     non-direchlet : {rf} = {rf} - [K]*{dvalue}
  !         direchlet : {rf} =            {dvalue}
  
  rf_tmp = (0.d0,0.d0) ! 2021.12.29
  do i=1,nsr
    dvalue_tmp(:,i) = dvalue(:) ! 2021.12.29 
  end do
  dvalue_tmp(:,nsr+1:nsr+2) = dvalue_mt(:,1:2) ! 2021.12.29
  
  do i=1,nline
      if (dirichlet(i)) then ! when line is for dirichlet boundary
        do j=1,nsr+nsr_mt  ! j=1 for ex, j=2 for ey polarization
             rf_tmp(i,j) = dvalue_tmp(i,j)
        end do
      else
        do j=1,nsr+nsr_mt  ! source type loop
          temp   =       A%D   (i   ) * dvalue_tmp(i,j)
          do ipos=A%INL(i-1)+1,A%INL(i)
            temp = temp + A%AL(ipos) * dvalue_tmp( A%IAL(ipos),j ) ! 2021.12.29
          enddo
          do ipos=A%INU(i-1)+1,A%INU(i)
            temp = temp + A%AU(ipos) * dvalue_tmp( A%IAU(ipos),j ) ! 2021.12.29
          enddo
          rf_tmp(i,j) = rf_tmp(i,j) - temp ! 2021.12.29
        end do ! source loop end 2021.09.14
      endif
  enddo ! nline loop
  !# set output
    rf(:,       1:nsr  ) = rf_tmp(:,    1:nsr  ) ! 2021.12.29
    rf_mt(:,nsr+1:nsr+2) = rf_tmp(:,nsr+1:nsr+2) ! 2021.12.29
    write(*,*) "check 4.3"
...
complex(8), intent(inout) ::  diag(nline)                  ! diagonal value   (i-th dof)

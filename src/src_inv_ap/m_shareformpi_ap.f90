...
!  CALL SHAREFACE(g_face,ip)                  ! see m_shareformpi.f90 2017.08.31
  CALL SHAREINVPARAAP(g_param_apinv,ip)       ! 2017.08.31 see below
...
!# Modified on 2017.08.31 for multisource inversion
!# Coded on 2017.06.08
subroutine SHAREINVPARAAP(g_param_apinv,ip) ! 2017.08.31
implicit none
integer(4),             intent(in)    :: ip
type(param_apinversion),intent(inout) :: g_param_apinv   ! 2017.08.31
...
integer(4)                            :: icomp           ! 2018.10.05
 ...
 call MPI_BCAST(g_param_apinv%nobs,    1, MPI_INTEGER4,0,MPI_COMM_WORLD,errno)    ! 2017.08.31
 call MPI_BCAST(g_param_apinv%nfreq,   1, MPI_INTEGER4,0,MPI_COMM_WORLD,errno)    ! 2017.08.31
 call MPI_BCAST(g_param_apinv%g_faceinfofile, 50, MPI_CHAR,0,MPI_COMM_WORLD,errno)! 2017.08.31
 call MPI_BCAST(g_param_apinv%g_initcondfile, 50, MPI_CHAR,0,MPI_COMM_WORLD,errno)! 2017.08.31
 call MPI_BCAST(g_param_apinv%nsr_inv, 1, MPI_INTEGER4,0,MPI_COMM_WORLD,errno)    ! 2017.08.31
 nobs     = g_param_apinv%nobs    ! 2017.08.31
 nfreq    = g_param_apinv%nfreq   ! 2017.08.31
 nsr_inv  = g_param_apinv%nsr_inv ! 2017.07.13

 if ( ip .ne. 0) then
  allocate(g_param_apinv%srcindex(nsr_inv) ) ! 2017.08.31
  allocate(g_param_apinv%obsinfo( nsr_inv) ) ! 2017.08.31
 end if
  call MPI_BCAST(g_param_apinv%srcindex,nsr_inv,MPI_INTEGER4, 0,MPI_COMM_WORLD,errno) !2017.08.31

 do j=1,nsr_inv       ! 2017.07.13
  call MPI_BCAST(g_param_apinv%obsinfo(j)%nobs_s,1,MPI_INTEGER4, 0,MPI_COMM_WORLD,errno) !2017.08.31
  nobs_s = g_param_apinv%obsinfo(j)%nobs_s                ! 2017.08.31
  if ( ip .ne. 0) then                                    ! 2017.08.31
   allocate(g_param_apinv%obsinfo(j)%ampfile(5,nobs_s) )  ! 2018.10.04
   allocate(g_param_apinv%obsinfo(j)%phafile(5,nobs_s) )  ! 2018.10.04
   allocate(g_param_apinv%obsinfo(j)%obsindex(2,5,nobs_s))! 2018.10.04
  end if                                                  ! 2017.08.31
  do icomp = 1,5                                          ! 2018.10.04
  do i=1,nobs_s                                           ! 2017.08.31
  call MPI_BCAST(g_param_apinv%obsinfo(j)%ampfile(icomp,i),50,MPI_CHAR, 0,MPI_COMM_WORLD,errno)     !2018.10.04
  call MPI_BCAST(g_param_apinv%obsinfo(j)%phafile(icomp,i),50,MPI_CHAR, 0,MPI_COMM_WORLD,errno)     !2018.10.04
  call MPI_BCAST(g_param_apinv%obsinfo(j)%obsindex(1,icomp,i),2,MPI_INTEGER4,0,MPI_COMM_WORLD,errno)!2018.10.04
  end do         ! 2017.07.13
  end do         ! 2018.10.04
 end do          ! 2017.07.13

 call MPI_BCAST(g_param_apinv%iflag_comp(1),5,MPI_INTEGER4,0,MPI_COMM_WORLD,errno) ! 2018.10.04

 call MPI_BCAST(g_param_apinv%errorfloor, 1,MPI_REAL8,0,MPI_COMM_WORLD,errno)      ! 2017.08.31
 call MPI_BCAST(g_param_apinv%nalpha,     1,MPI_INTEGER4,0,MPI_COMM_WORLD,errno)   ! 2017.09.11
 call MPI_BCAST(g_param_apinv%ialphaflag, 1,MPI_INTEGER4,0,MPI_COMM_WORLD,errno)   ! 2017.09.11

 if ( ip .eq. 0 ) write(*,'(a)') " ### SHAREINVPARA  END!! ###" ! 2020.09.17
return
end

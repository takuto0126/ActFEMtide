...
!############################################## calroughness
!# 2018.01.22
subroutine CALROUGHNESS(g_param_apinv,h_model,g_model_ref,rough1,rough2,BM,R)
use param_apinv
use modelpart
use jacobian_ap
use caltime
...
!#[1]## set
 ...
 itype_roughness = g_param_apinv%itype_roughness
 iboundflag      = g_param_apinv%iboundflag      !0:off,1:cut,2:trnsformed model
...
!#[2]## cal roughness
   if (     itype_roughness .eq. 1 ) then ! SM: Smoothest model
    ...
    CALL CALROUGHNESS_MS(g_param_apinv,h_model,g_model_ref,rough1 ) ! 2017.12.13
   else if (itype_roughness .eq. 3 ) then ! MSG: Minimum support gradient
    CALL CALROUGHNESS_MSG(R,g_param_apinv,h_model,g_model_ref,rough1 )! 2017.12.13
   end if
...
!#[3]## cal roughness of
  if (iboundflag .eq. 2 ) then
   ki     = h_model
   ki_ref = g_model_ref
   call TRANSMODELX(ki,    g_param_apinv,1) ! model -> ki
   call TRANSMODELX(ki_ref,g_param_apinv,1) ! model -> ki
   !#
   if (     itype_roughness .eq. 1 ) then ! SM: Smoothest model
    CALL CALROUGHNESS_SM(BM,ki,ki_ref,rough2 )              ! 2018.01.22 m_modelroughness.
   else if (itype_roughness .eq. 2 ) then ! MS: Minimum support
    CALL CALROUGHNESS_MS(g_param_apinv,ki,ki_ref,rough2 )   ! 2018.01.22
   else if (itype_roughness .eq. 3 ) then ! MSG: Minimum support gradient
    CALL CALROUGHNESS_MSG(R,g_param_apinv,ki,ki_ref,rough2 )! 2018.01.22
   end if
   !#
  end if
...
!# Coded on 2017.12.12
subroutine GENBMI_MS(g_model,g_model_ref,g_param_apinv,BM,BMI,it,ialphaflag)
use modelpart    ! m_modelpart.f90
use matrix       ! m_matrix.f90
use param_apinv  ! m_param_apinv.f90
use jacobian_ap  ! m_jacobian_ap.f90 for TRANSMODELX 2018.02.04
...
type(watch)                           :: t_watch
 ...
 !#[1]## set
 beta         = g_param_apinv%beta
 nmodel       = g_model%nmodel
 allocate(logrhomodel1(nmodel),logrhomodel2(nmodel))
 iboundflag   = g_param_apinv%iboundflag !0:off,1:cut,2:trnsformed model, 2018.02.04
 BMIdenomflag = g_param_apinv%BMIdenomflag ! 1:model, 2:chi are used in BMI dnominator
 ...
 !#[3]## when ibound = 2, transfer model to ki, 2018.02.04
  if (iboundflag .eq. 2 .and. BMIdenomflag .eq. 2 ) then  ! 2018.02.04
   ...
   call TRANSMODELX(ki,    g_param_apinv,1) ! model -> ki m_jacobian_ap.f90
   call TRANSMODELX(ki_ref,g_param_apinv,1) ! model -> ki m_jacobian_ap.f90
...
!# coded on 2017.12.13
subroutine GENBMI_MSG(g_model,g_model_ref,g_param_apinv,R,RI,BM,BMI,it,ialphaflag)
use modelpart    ! m_modelpart.f90
use matrix       ! m_matrix.f90
use param_apinv  ! m_param_apinv.f90
use caltime      ! m_caltime.f90     2017.12.18
use jacobian_ap  ! m_jacobian_ap.f90 2018.02.04
implicit none
type(param_apinversion),intent(in)    :: g_param_apinv
integer(4),             intent(in)    :: it
integer(4),             intent(in)    :: ialphaflag ! 2017.12.25
...
!#[1]## set
 ...
 nmodel       = g_model%nmodel
 iboundflag   = g_param_apinv%iboundflag   ! 0:off,1:cut,2:trnsformed model, 2018.02.04
 BMIdenomflag = g_param_apinv%BMIdenomflag ! 1:model, 2:chi are used in BMI 2018.02.05
...
subroutine CALROUGHNESS_MSG(R,g_param_apinv,g_model,g_model_ref,rough)
use param_apinv
use modelpart
use matrix
...
!# coded on 2017.12.13
!# roughness for minimum support
subroutine CALROUGHNESS_MS(g_param_apinv,g_model,g_model_ref,rough1)
use param_apinv
use modelpart
use caltime  ! 2017.12.22
implicit none
...
!#[4]## calculate DQ [nphys2,nmodel]
  ...
  call mulreal_crs_crs_crs(D,Q,DQ,iflag_mkl=1) ! see m_matrix.f90 2022.01.27 iflag_mkl added
  ...
  call watchstart(t_watch)        ! 2017.09.06
  call mulreal_crs_crs_crs(QT,DQ,QTDQ,iflag_mkl=1) ! 2022.01.27 iflag_mkl added connectivity of models
  call watchstop(t_watch)        ! 2017.09.06

...
subroutine forward_mdminv(A,h_mesh,l_line,doftot,nsr,bs,omega,sparam,g_param,&
			&    g_param_inv,g_cond,PT,ut) ! 2017.10.31
use mesh_type
use iccg_var_takuto
use line_type
use param
...
type(real_crs_matrix),   intent(in)     :: PT      ! 2017.05.15
complex(8),              intent(out)    :: bs(doftot,nsr)  ! 2017.07.13
type(global_matrix),     intent(inout)  :: A ! see m_iccg_var_takuto.f90
type(complex_crs_matrix),intent(out)    :: ut    ! 2017.05.15
!type(complex_crs_matrix) :: ut    ! 2017.05.15
!--------------- internal variants
...
CALL GENMAT(h_mesh,l_line,A,b_vec,omega,sparam,g_param,g_cond,g_param_inv) ! 2017.07.13

!#[4-3]## Copy upper triangle to lower driangle
CALL COPY_UL_ICCG12(A)
...
!#[7]## Solve
call solvePARDISOinv(doftot,nsr, A,b_vec,bs,PT,ut) ! 2017.07.13

write(*,*) "### forward_inv END !! ###"
return
end subroutine forward_mdminv
!########################################
! modified on 2017.07.13 for multiple sources
...
subroutine GENMAT(h_mesh,l_line,A,b_vec,omega,sparam,g_param,g_cond,g_param_inv)
...
use  param
use  param_mdminv       ! 2017.10.31
use  constants,   only:pi,dmu          ! see m_constants.f90, 2017.07.13
...
real(8),              intent(in)    :: omega
complex(8),           intent(out)   :: b_vec(l_line%nline,g_param_inv%nsr_inv)!20170713
...
!#[0]## set
  ...
  srcindex = g_param_inv%srcindex   ! 2017.07.13

...
do ii=1,h_mesh%ntet
 x3(:)=0.d0
 do j=1,4
  x3(1:3)=x3(1:3)+h_mesh%xyz(1:3,h_mesh%n4(ii,j))/4.d0
   ! x,y,z coordinate of center of gravity
 end do
 do k=1,glist%nz
  if (glist%zgrd(k) .lt. x3(3) .and. x3(3) .lt. glist%zgrd(k+1)) then
    iz=k
  do j=1,glist%ny
  if (glist%ygrd(j) .lt. x3(2) .and. x3(2) .lt. glist%ygrd(j+1)) then
    iy=j
   do i=1,glist%nx
   if (glist%xgrd(i) .lt. x3(1) .and. x3(1) .lt. glist%xgrd(i+1)) then
    ix=i ; goto 100
   end if
   end do
  end if
...
allocate (h_mesh%cmodel(h_mesh%nmodel))

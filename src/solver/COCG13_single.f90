! This program was revealed to be COCG mehod code, then renamed to COCG13 on Feb. 17, 2016
! note in COCG that only inner product is different from CG method
!
...
!     Conjugate Gradient iterative method with preconditioning.
      subroutine COCG13_single(NP,  NPL, NPU,                               &
...
!	  write(*,*) "D(j)=",D(j),"B(j)=",B(j)
        ...
        enddo
        WW(j,R)= WVAL     !  WW(j,R) is {r0}
      enddo
!######################### debug
!      test0=0.d0
!      do i= 1, NP
!	 test0= test0 + WW(i,R)*dconjg(WW(i,R))
!      enddo
!	write(*,*) "SUM of WW(i,Z)=",test0
!######################### debug
...
!##  when the right hand side equals to 0
	...
	write(*,*) "BNRM2=",BNRM2
      ...
      do iter= 1, MAXIT !----------------------------------------------------- Iteration start
!C*********************************************
!C      +----------------+
!C#[1]##| {z}= [M^-1]{r} |
!C      +----------------+
...
!	  write(*,*) "DD(i)=",DD(i),"SW=",SW,"i=",i
        WW(i,Z)= WW(i,Z) - DD(i) * SW   ! WW(i,Z) is Z0 = [M^-1]{r0}
       enddo
      endif
!######################### debug
!      test0=0.d0
!      do i= 1, NP
!	 test0= test0 + WW(i,Z)*dconjg(WW(i,Z))
!      enddo
!	write(*,*) "SUM of WW(i,Z)=",test0
!######################### debug
...
!C#[3]## +---------------+
!C       | {RHO}= {r}{z} |  ! In COCG method, RHO_i={r_i}^T{z_i}
!C       +---------------+
      RHO0= (0.d0, 0.d0)
      do i= 1, NP
...
!C       +-----------------------------+
	 ...
	 do i= 1, NP
!	  write(*,*) "WW(i,Z)=",WW(i,Z)
	  WW(i,P)= WW(i,Z)  ! if ITER =1, P0 = z0 = [M^-1]{r0}
	 enddo
	else
	 BETA= RHO / RHO1   ! if ITER > 1 P_new = z + beta * P_old
...
!C#[5]## +-------------+
!C       | {q}= [A]{p} |  Matrix vector multiplication
!C       +-------------+
	 ...
	 enddo
	 WW(j,Q)= WVAL      ! q = [A]{p} is calcuated
      enddo
...
!C       +---------------------+
      C10= (0.d0, 0.d0)
      do i= 1, NP
	 C10= C10 + WW(i,P)*WW(i,Q)   ! inner product, {p}^T{q}
      enddo
      C1=C10
      ALPHA= RHO / C1               ! alpha = RHO / {p}^T{q}
...
!C#[8]## check if RESID is small enough or not
	 ...
	 DNRM20= DNRM20 + WW(i,R)*conjg(WW(i,R)) ! DNRM20 (real)
      enddo
	DNRM2=DNRM20
	RESID= dsqrt(DNRM2/BNRM2) ! Resid = ||R||/||R0||

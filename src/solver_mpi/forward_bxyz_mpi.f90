...
type(param_forward),intent(in)      :: g_param ! 2024.02.05
type(param_source), intent(in)      :: sparam
type(param_cond),   intent(in)      :: g_cond
...
integer(4)                          :: nodes,node ! 2024.03.16
...
write(*,*) "fxyz_file",g_param%fxyz_file
write(*,*) "vxyz_file",g_param%vxyz_file

inum=(0,1)
 
 node  = h_mesh%node    ! node for em3d.msh    2024.03.16
 nodes = g_param%nodes  ! node for ocean_meesh 2024.03.16
 allocate(vxyz(3,node),fxyz(3,node)) ! 2024.03.16
  
 call readfxyz(nodes,node,fxyz,g_param%fxyz_file) ! 2024.03.16
 call readvxyz(nodes,node,vxyz,g_param%vxyz_file) ! 2024.03.16
...
!#
  ...
  j = h_mesh%n4flag(iele,2)                 ! 2017.09.29
  if ( j .eq. 1 ) sigma=g_cond%sigma_air    ! 2017.09.29
  if ( j .eq. 2 ) sigma=3.33 ! 2024.02.05
!  if ( j .eq. 3 ) sigma=0.01 ! 2024.02.05 slab
  if ( j .ge. 3 ) then                      ! 2024.02.05
...
!  if ( h_mesh%n4flag(iele,2) .ge. 4 ) sigma=0.01d0   ! ocean
 ...
 do i=1,6
   ii=l_line%n6line(iele,i)*idirection(i)
   b_vec(ii,1) = b_vec(ii,1)+S2(i)!(mV/km)
   ! write(6,*)rf(ii,1)

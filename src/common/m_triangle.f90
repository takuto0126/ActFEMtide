...
write(*,*) "### allocate_2Dgrid_list END!! ###" ! 2017.09.28
return
end subroutine
...
write(*,*) "### gen2Dgridforlist END!! ###" ! 2017.09.28
return
end subroutine
...
! if (mod(ii,1000) .eq. 0 )write(*,*) ii,"/",h_mesh%ntri,"x2(1:2)",x2(1:2)
  do j=1,glist%ny
...
ny=glist%ny
  ...
  stop
  101 continue
  !write(*,'(a,2i5,a,2g15.7)') "ix,iy",ix,iy,"x2",x2(1:2)
  ...
  !#[2]## construct sum of nelist for search x3(1:3)
   ...
   !
   if (  elist%ne .eq. 0 ) then ! 2024.03.07
     iside=iside+1
     if ( iside > min(nx,ny) ) goto 99 ! 2024.03.07
...
!write(*,*) "findtriwithgrid end!!"
return
99 continue
write(*,*) "iside",iside," exceedes 10 or min(nx,ny) from nx,ny",nx,ny
stop
end subroutine
...
if (present(nelist) ) ii=nelist%list(i)
 ...
 !#[1]## generate n(1:4)
  ...
  do j=1,3
   !write(*,'(a,a,2f9.3,a)') " element x,y","(",elm_xyz(1:2,j),")"
  end do
 !# calculate a(1:3)

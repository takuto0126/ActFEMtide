...
type real_crs_matrix ! compressed row matrix
 integer(4) :: nrow=0
 integer(4) :: ncolm=0 ! added on 2017.05.11 (required for mat-mat multiplication)
 integer(4) :: ntot=0
...
type real_ccs_matrix ! compressed colmun matrix, added on 2017.05.10
 integer(4) :: nrow=0 ! added on 2017.05.11 (required for mat-mat multiplication)
 integer(4) :: ncolm=0
 integer(4) :: ntot=0
 integer(4),dimension(:),allocatable :: item !item(1:ntot) :colmun id for every entry for
 integer(4),dimension(:),allocatable :: stack!stack(0:nrow) accumulated # of row members
...
!write(*,*) "size(vout)",size(vout),"crsmat%nrow",crsmat%nrow
!write(*,*) "size(cv)",size(cv),"doftot",doftot
vout(:)=(0.d0,0.d0)
do irow=1,crsmat%nrow
...
!############################################### add_crs_crs_crs2
! Coded on 2022.01.05
subroutine add_crs_crs_crs2(crs1,crs2,crs3)
  implicit none
  ...
  integer(4) :: i,icol,j
  real(8),allocatable,dimension(:,:) :: full
...
if (ncolm1 .ne. ncolm2) goto 99
   ...
   !#[2]##
    allocate(full(nrow1,ncolm1))
    full=0.d0

  ! add crs1
  do i=1,nrow1
   do j=stack1(i-1)+1,stack1(i)
    icol=item1(j)
    full(i,icol)=full(i,icol)+val1(j)
   end do
  end do

  ! add crs2
  do i=1,nrow2
    do j=stack2(i-1)+1,stack2(i)
     icol=item2(j)
     full(i,icol)=full(i,icol)+val2(j)
    end do
   end do

   !# full to crs
   call conv_full2crs(full,nrow1,ncolm2,crs3,1.d-10)
...
!#[2]## generate crs
call deallocate_complex_crsmat(crs) ! 2021.12.30
crs%ntot = ntot
 allocate(crs%val(ntot),crs%item(ntot),crs%stack(0:nrow))
 crs%stack(0)=0
 do i=1,nrow
...
!######################################################### vertical_combine_real_crs
! 2021.01.05
subroutine vertical_combine_real_crs(crs1,crs2,crs3)
  implicit none
  type(real_crs_matrix),intent(in)  :: crs1,crs2
  type(real_crs_matrix),intent(out) :: crs3
  type(real_crs_matrix)             :: crs
 
  if ( crs1%ncolm .ne. crs2%ncolm ) then
    write(*,*) "GEGEGE! crs1%ncolm=",crs1%ncolm,"crs2%ncolm",crs2%ncolm
    stop
  end if
  crs3%ntot  =crs1%ntot + crs2%ntot
  crs3%nrow  =crs1%nrow + crs2%nrow
  crs3%ncolm =crs1%ncolm
  if ( allocated(crs3%stack) ) then
    deallocate(crs3%stack,crs3%item,crs3%val)
  end if
  allocate(crs3%stack(0:crs3%nrow))
  allocate(crs3%item( crs3%ntot))
  allocate(crs3%val(  crs3%ntot))
  crs3%stack(0:crs1%nrow) =crs1%stack
  crs3%item( 1:crs1%ntot) =crs1%item(1:crs1%ntot)
  crs3%val(  1:crs1%ntot) =crs1%val( 1:crs1%ntot)
  crs=crs2
!  write(*,*) "crs1%nrow",crs1%nrow
!  write(*,*) "crs2%nrow",crs2%nrow
!  write(*,*) "crs3%nrow",crs3%nrow
  call combine_real_crs_mat(crs3,crs2,crs1%nrow+1)
...
implicit none
integer(4),           intent(in)    :: irow ! starting row index
...
!------------------------------------- 2017.12.25
subroutine mulreal_crs_crs_crs(crsmat1,crsmat2,crsout,T,iflag_mkl) ! T is added on 2017.12.25
implicit none
...
integer,     optional,intent(in)  :: iflag_mkl ! 0:normal, 1:mkl library 2022.01.27
...
!================================================
if (present(iflag_mkl) .and. iflag_mkl .eq. 1 ) iflag = 1 ! 2022.01.27
...
! write(*,*) "conv_ccs2crs end!"
 ...
 ! write(*,*) "start!"
 call mkl_dcsrmultcsr(trans,request,sort,m1,n1,n2,a,ja,ia,b,jb,ib,c,jc,ic,nzmax,info)
 !write(*,*) "first end!!"
 request=0
...
integer(4) :: i,j,k,l
integer(4) :: TID,OMP_GET_THREAD_NUM ! 2022.01.13
...
!#[2]## multiply
 ...
 k=k+1
  ...
  ntotoutmax = int(nrowcrs * maxscale(k)) * ncolmccs ! 2022.01.13
...
!#[2]## gen transverse ccs
call deallocate_complex_ccsmat(ccs) !2021.12.30
ccs%nrow  = ncolm
ccs%ncolm = nrow
ccs%ntot  = ntot
allocate(ccs%stack(0:ccs%ncolm),ccs%item(ntot),ccs%val(ntot))
...
!############################################### realcrsout
!# coded on 2017.05.17
subroutine realcrsout(crsout,idev)
implicit none
type(real_crs_matrix),intent(in) :: crsout
integer(4),optional,intent(in) :: idev
integer(4):: iunit=6
if (present(idev)) iunit=idev
write(iunit,*) "size(item)=",size(crsout%item)
write(iunit,*) "size(stack)=",size(crsout%stack)
write(iunit,*) "crs%nrow=",crsout%nrow
write(iunit,*) "crs%ncolm=",crsout%ncolm
write(iunit,*) "crs%ntot=",crsout%ntot
write(iunit,*) "crs%stack=",crsout%stack
write(iunit,*) "crs%item=",crsout%item
write(iunit,*) "crs%val=",crsout%val

return
end subroutine
!############################################### complexcrsout
!# coded on 2022.01.05
subroutine complexcrsout(crsout,idev)
  implicit none
...
!############################################### dewallocate real_crsmatrix
!# 2021.12.30
subroutine deallocate_complex_crsmat(crsmat)
  implicit none
  type(complex_crs_matrix),intent(inout) :: crsmat
...
!############################################### deallocate complex_ccsmatrix
!# 2021.12.30
  subroutine  deallocate_complex_ccsmat(ccsmat)
    implicit none
    type(complex_ccs_matrix),intent(inout) :: ccsmat
    
    ccsmat%ntot = 0.d0
    ccsmat%nrow = 0.d0
    ccsmat%ncolm = 0.d0
    if (allocated(ccsmat%stack)) deallocate(ccsmat%stack)
    if (allocated(ccsmat%item) ) deallocate(ccsmat%item)
    if (allocated(ccsmat%val)  ) deallocate(ccsmat%val)

...
type param_forward
 ...
 character(70) :: meshctlfile   ! mesh control file  2024.03.08
 character(70) :: ocean_meshfile
 character(70) :: vxyz_file
 character(70) :: fxyz_file 
 ...
 real(8)       :: depth_xy     ! 2024.03.09

 !#
 integer(4) :: iflagspherical=0 ! 0 for normal, 1 for spherical coordinate 2024.03.09
 ...
 !#[6-2]## when ixyflag = 2, extrude mesh data are required 2024.03.07
 ...
 !# ocean 
  integer(4) :: nodes ! node for ocean_mesh ! 2024.03.16
...
type param_source
 ...
 character(70),allocatable,dimension(:) :: sourcename ! 2017.07.11
...
read(input,10) c_param%ocean_meshfile
write(*,   41) " ocean meshfile : ",trim(c_param%ocean_meshfile)
read(input,10) c_param%meshctlfile                                 ! 2024.03.08
write(*,   41) " mesh ctl file  : ",trim(c_param%meshctlfile)      ! 2024.03.08
read(input,10) c_param%vxyz_file
write(*,   41) " vxyz file : ",trim(c_param%vxyz_file)
read(input,10) c_param%fxyz_file
write(*,   41) " fxyz file : ",trim(c_param%fxyz_file)
...
!# mesh info
 ...
 write(*,*) "" ! 2020.09.29
 write(*,'(a,4f15.7)') " xbound =",c_param%xbound(1:4) ! 2021.09.02
 write(*,'(a,4f15.7)') " ybound =",c_param%ybound(1:4) ! 2021.09.02
 write(*,'(a,4f15.7)') " zbound =",c_param%zbound(1:4) ! 2021.09.02
 write(*,*) "" ! 2021.09.02
!# observatory info
...
!# lonlatflag
 ...
 if (c_param%lonlatflag .eq. 0) then
  read(input,*) ! 2024.02.08
 else if ( c_param%lonlatflag .eq. 1) then ! lonlat
  read(input,*) c_param%UTM           ! 10->* 2021.09.02
  write(*,*) "UTM zone : ",c_param%UTM ! 2020.09.29
elseif( c_param%lonlatflag .eq. 2) then
   read(input,*)
end if
...
write(*,*) "" !2021.09.29
 ...
 do i=1, nobs
   ...
   else if (c_param%lonlatflag .eq. 1 ) then
     read(input,*) (c_param%lonlataltobs(j,i),j=1,3)
...
!#[2]# xy reading  2017.10.11
 write(*,*) ""
 write(*,*) "< input ixyflag: 0 for nothing, 1 for nx ny plan view, 2:ntrik >"
 ...
 if ( c_param%ixyflag .eq. 1) then ! 2017.10.11
   ...
   read(input,*) c_param%ixydepth       ! 1 for const. depth, 2 for seafloor
   if (c_param%ixydepth .eq. 1) then     ! 204.03.11
     write(*,*) "Input depth [km] for xy plain data" ! 204.03.11
     read(input,*) c_param%depth_xy          ! 204.03.11
   end if     
   read(input,'(a)') c_param%pokimsh    ! 2024.03.09       
   read(input,'(a)') c_param%ki23dfile      ! 2024.03.08
   write(*,*) "xyfilehead = ",c_param%xyfilehead
   write(*,*) "nx,ny =",c_param%nx,c_param%ny
   write(*,'(a,a)') " ki23dfile = ",trim(c_param%ki23dfile) ! 2024.03.08
 end if                                ! 2017.10.11
 if ( c_param%ixyflag .eq. 2) then     ! 2017.10.11
   read(input,*) c_param%xyfilehead     ! 2021.09.02
   read(input,'(a)') c_param%pokimsh    ! 2024.03.09       
   read(input,'(a)') c_param%ki23dfile      ! 2024.03.08
   write(*,'(a,a)') " xyfilehead = ",trim(c_param%xyfilehead)
   write(*,'(a,a)') " ki23dfile = ",trim(c_param%ki23dfile) ! 2024.03.08
 end if                                ! 2017.10.11
...
!#[3]# read source param
 ...
 !# xy input
 else if (sparam%lonlatflag .eq. 2 .or. sparam%lonlatflag .eq. 0) then ! xyz should be modified later 2024.02.08

! pointer is deleted on 2017.09.08
...
integer(4),allocatable,dimension(:)   :: line_stack ! line_stack(0:node)
integer(4),allocatable,dimension(:,:) :: line       !line(1:2,i) is start and end node
integer(4),allocatable,dimension(:)   :: line_item
...
implicit none
type(line_info),intent(out)           :: l_line
integer(4),     intent(in)            :: nodeg, ne, enod,n4(ne,enod)
integer(4),     parameter             :: maxnline=6000000
integer(4),allocatable,dimension(:)   :: line_item_work,line_stack
integer(4)                            :: i,j,k,l,n1,n2,nline,icount,n1o,n2o,iele
integer(4),     parameter             :: maxnele=300
integer(4),allocatable,dimension(:,:) :: node2ele ! node2ele(j,i) is element id of j-th element of i-th node
integer(4),allocatable,dimension(:) :: nele        ! nele(i) is # of element including i-th node
allocate(nele(nodeg),node2ele(maxnele,nodeg))
allocate(line_item_work(maxnline),line_stack(0:nodeg))
...
!#[0] copy
allocate(line_stack(0:l_line%node), line_item(1:l_line%nline))

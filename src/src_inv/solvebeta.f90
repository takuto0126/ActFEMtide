! Coded on 2017.05.17 to obtaine beta
!INCLUDE 'mkl_pardiso.f90'
subroutine solvebeta(A,b_vec,doftot,xout)
use mkl_pardiso
use matrix
IMPLICIT NONE
!==========================
...
real(8),              intent(in)  :: b_vec(doftot) ! directly used in PARDISO
real(8),              intent(out) :: xout(doftot) ! direct output form PARDISO
!==========================
...
nrhs = 1       ! number of right hand side vector
maxfct = 1 
mnum = 1
ALLOCATE(ia(n + 1))
ALLOCATE(ja(nnz))
...
!write(*,*) "solvePARDISO start!!" !2020.09.29
